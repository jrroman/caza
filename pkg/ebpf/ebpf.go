...
func ProcessEvents(ec chan *NetworkPair, networks map[string]*net.IPNet) {
	...
	for {
		eventData, ok := <-ec
		if !ok {
			log.Println("channel closed, stop event processing")
			return
		}
		pair := createNetworkPairRelation(networks, eventData)
		proximity := reportNetworkProximity(pair)
		log.Printf("%-15s %-6d -> %-15s %-6d %-6s",
			pair.src.ip, pair.src.port, pair.dst.ip, pair.dst.port, proximity)
	}
}
...
func readFromRingBuffer(ctx context.Context, rd *ringbuf.Reader, ec chan *NetworkPair) {
	for {
		select {
		default:
		...
		case <-ctx.Done():
			log.Println("context finished closing events channel")
			close(ec)
			return
		}
	}
...
func LoadAndRun(ctx context.Context, ec chan *NetworkPair) {
